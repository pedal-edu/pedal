...
def wrong_target_is_list():
    ...
    for loop in for_loops:
        ...
        if iter_prop.std_ast_name == "Name" and iter_prop.data_type == "List":
            explain('The variable <code>{0!s}</code> is a list and should not be placed in the iteration variable slot'
                    ' of the "for" block<br><br><i>(target_is_list)<i></br>.'.format(iter_prop.id))
    return False
...
def wrong_list_repeated_in_for():
    ...
    for loop in for_loops:
        iter_prop = loop.target
        list_prop = loop.iter
        if iter_prop.std_ast_name == "Name" and list_prop.std_ast_name == "Name" and iter_prop.id ==\
                list_prop.id and iter_prop.data_type == "List":
            explain('The <code>{0!s}</code> variable can only appear once in the "for" block <br><br><i>'
                    '(list_repeat)<i></br>'.format(list_prop.id))
    return False
...
def missing_iterator_initialization():
    ...
    for loop in for_loops:
        list_prop = loop.iter
        if list_prop.std_ast_name != "List" and (list_prop.data_type != "List" or def_use_error(list_prop)):
            if list_prop.id == "___":
                explain("The slot to hold a list in the iteration is empty.<br><br><i>"
                        "(no_iter_init-blank)<i></br>".format(list_prop.id))
            else:
                explain("The variable <code>{0!s}</code> is in the list slot of the iteration but is not a list."
                        "<br><br><i>(no_iter_init)<i></br>".format(list_prop.id))
            return True
    return False
...
def wrong_iterator_not_list():
    ...
    for loop in for_loops:
        list_prop = loop.iter
        if list_prop.std_ast_name != "List" and list_prop.data_type != "List" and list_prop.id != "___":
            if list_prop.std_ast_name == "Name":
                explain("The variable <code>{0!s}</code> has been set to something that is not a list but is placed in "
                        "the iteration block that must be a list.<br><br><i>"
                        "(iter_not_list)<i></br>".format(list_prop.id))
                return True
    return False
...
def missing_for_slot_empty():
    ...
    for loop in for_loops:
        list_prop = loop.iter
        iter_prop = loop.target
        if list_prop.std_ast_name == "Name" and list_prop.id == "___":
            is_missing = True
            break
        if iter_prop.std_ast_name == "Name" and iter_prop.id == "___":
            is_missing = True
            break
    if is_missing:
...
def wrong_target_reassigned():
    ...
    for assignment in assignments:
        ...
        if is_reassigned:
            break
    if is_reassigned:
        explain("The variable <code>{0!s}</code> has been reassigned. The iteration variable shouldn't be reassigned"
                "<br><br><i>(target_reassign)<i></br>".format(off_prop))
