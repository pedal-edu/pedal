    ...
    if matches:
        for match in matches:
            ...
            if _item1_.id != _item2_.id:
                return explain_r(message.format(_item1_.id, _item2_.id), code, tldr=tldr)
    return False
        ...
        for match01 in matches01:
            ...
            if submatches01:
                for submatch01 in submatches01:
                    ...
                    if not matches02:
                        return explain_r(message.format(_sum_.id), code, tldr=tldr)
    return False
            ...
            if submatches:
                for submatch in submatches:
                    ...
                    if _total_.id != _count_.id != _average_.id and _total_.id != _average_.id:
                        return explain_r(message.format(_total_.id, _count_.id), code, tldr=tldr)
...
def wrong_conversion_10_2():
    ...
    for match in matches:
        ...
        if matches02:
            return False
        return explain_r(message.format(_target_.id), code, tldr=tldr)
    return False
            ...
            if submatches:
                for submatch in submatches:
                    ...
                    if len(__expr__sub.find_all("Name")) == 0:
                        return explain_r(message.format(_assign_.id), code, tldr=tldr)
    return False
...
def all_labels_present():  # TODO: make sure it's before the show, maybe check for default values
    ...
    if (not match) or (not match02) or (not match03):
        return gently_r(message, code, tldr=tldr)
    return False
