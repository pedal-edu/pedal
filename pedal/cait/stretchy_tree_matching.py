...
class StretchyTreeMatcher:
    ...
    def find_matches(self, ast_or_code, filename="__main__", check_meta=True, prev_match=None):
        """
        Args:
            prev_match:
            ast_or_code (str or AstNode): The students' code or a valid AstNode from
        ...
        matches = self.any_node_match(explore_root, other_root,
                                      check_meta=check_meta, prev_match=prev_match)
        explore_root.field = explore_root_old_field
        other_root.field = other_root_old_field
        return matches

    def any_node_match(self, ins_node, std_node, check_meta=True, cut=False, prev_match=None):
        """
        ...
        matching = self.deep_find_match(ins_node, std_node, check_meta, prev_match=prev_match)
        ...
        for std_child in std_node.children:
            matching_c = self.any_node_match(ins_node, std_child, check_meta=check_meta, cut=cut, prev_match=prev_match)
            if matching_c:
    ...
    def deep_find_match(self, ins_node, std_node, check_meta=True,
        ...
        Args:
            ...
            prev_match: If this was part of a previous match...
        ...
        return target_func(ins_node, std_node, check_meta, prev_match=prev_match)

    # noinspection PyPep8Naming
    def deep_find_match_Name(self, ins_node, std_node, check_meta=True, prev_match=None):
        """
        ...
        if match[_VAR] and meta_matched:  # if variable
            if type(std_node.astNode).__name__ == "Name":
                return self.deep_find_match_generic(ins_node, std_node,
                                                    check_meta=check_meta, ignores=["ctx"], prev_match=prev_match)
        # could else return False, but shallow_match_generic should do this as well
            ...
            mapping.merge_map_with(prev_match)
    ...
    # noinspection PyPep8Naming
    def deep_find_match_BinOp(self, ins_node, std_node, check_meta=True, prev_match=None):
        """
        ...
        if is_generic:
            return self.deep_find_match_generic(ins_node, std_node, check_meta, prev_match=prev_match)
        else:  # this means that the node is clearly commutative
            return self.deep_find_match_binflex(ins_node, std_node, False, prev_match=prev_match)

    # noinspection PyMethodMayBeStatic
    def binflex_helper(self, case_left, case_right, new_mappings, base_mappings, prev_match=None):
        """
        ...
        Args:
            ...
            prev_match: A mapping passed down from an initial match
        Returns:
            None
        """
        if case_left and case_right:
            for case_l in case_left:
                new_map = base_mappings[0].new_merged_map(case_l).new_merged_map(prev_match)
                for case_r in case_right:
                    both = new_map.new_merged_map(case_r)
                    if not both.has_conflicts():
                        new_mappings.append(both)

    def deep_find_match_binflex(self, ins_node, std_node, check_meta=False, prev_match=None):
        """
        ...
        if base_mappings:
            ...
            self.binflex_helper(case_left, case_right, new_mappings, base_mappings, prev_match=prev_match)
    ...
    def deep_find_match_Expr(self, ins_node, std_node, check_meta=True, prev_match=None):
        """
        ...
        Args:
            ...
            prev_match: An AstMap from a previous matching run
        ...
        if not self.metas_match(ins_node, std_node, check_meta):
            return []
        mapping = AstMap() if prev_match is None else prev_match
    ...
    def deep_find_match_generic(self, ins_node, std_node, check_meta=True, ignores=None, prev_match=None):
        """
        ...
        Args:
            ...
            prev_match: a map from a previous match
